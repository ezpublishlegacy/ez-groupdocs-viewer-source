...
$http = eZHTTPTool::instance (); 
		...
		if(!isset($_SESSION['gdvcreatetable']) || !$_SESSION['gdvcreatetable']){
			$query = 'CREATE TABLE IF NOT EXISTS `gdv` (
...
if( $http->hasVariable('del_id') )  {
	...
	$query = 'DELETE FROM gdv WHERE id='.(int)$del_id; 
	$db -> arrayQuery( $query );
	return $module->redirectTo('/groupdocsviewer/config');
}
...
if( $file_id != '' ) 
{
		...
		// assign hook_id
		$HookId = GroupDocsViewer::getMaxId();
		$hook = '#gdviewer'.($HookId+1).'#';// as no records show zero
		// generate new data object 
		$GDObject = GroupDocsViewer::create( $file_id, $hook, $width, $height);
		...
		$count = GroupDocsViewer::getListCount(); 
	...
	eZDebug::writeNotice( $statusMessage, 'groupdocsviewer:groupdocsviewer/config.php' ); 
	eZDebug::writeDebug( $statusMessage, 'groupdocsviewer:groupdocsviewer/config.php' ); 
	eZDebug::writeWarning( $statusMessage, 'groupdocsviewer:groupdocsviewer/config.php' ); 
	eZDebug::writeError( $statusMessage, 'groupdocsviewer:groupdocsviewer/config.php' );
}
/////////////////////////////////////////// form ended ////////////////////////////////////////////////
// Get list of file from DB
$dataArray = array();
$query = 'SELECT * FROM gdv'; 
...
$t = $tpl->fetch('design:groupdocsviewer/config.tpl');

$Result['content'] = $t; //main tpl file to display the output

$Result['left_menu'] = "design:groupdocsviewer/leftmenu.tpl"; 

$Result['path'] = array( array( 
	'url' => 'groupdocsviewer/config',
	'text' => 'Groupdocs Viewer' 
...
$groupdocsviewerINI = eZINI::instance( 'groupdocsviewer.ini' ); 
 
$gdvDebug = $groupdocsviewerINI->variable( 'GDVExtensionSetting','JacDebug' ); 
 
// If Debug is activated do something 
if( $gdvDebug === 'enabled' ) 
    echo 'groupdocsviewer.ini: [GDVExtensionSetting] GdvDebug=enabled';
